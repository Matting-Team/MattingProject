...
# MSE & SAD(Sum Absolute Diff)
##################################################################
def evaluation_testset(generator, dataset, device):
    generator.eval()
    mse_criterion = nn.MSELoss()

    current_total_mse = 0.0
    current_total_sad = 0.0
    for data in tqdm.tqdm(dataset):
        ...
        mse = mse_criterion(gt, alpha)
        sad = SAD(gt, alpha)
        current_total_mse += mse
        current_total_sad += sad


    print("Total mse is {} Total sad is {}".format(current_total_mse / len(dataset), current_total_sad / len(dataset)))
